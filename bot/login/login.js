...
const titles = [
        ...
        [
                "Aizen V2 - Advanced Messenger Bot"
        ]
];
...
let subTitle = `GoatBot@${currentVersion} - Made by mdajmaul (Aizen V2)`;
const subTitleArray = [];
...
const author = ("Made by mdajmaul (Aizen V2)");
const srcUrl = ("Source code: https://github.com/mdajmaul/Aizen V2");
...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        log.info("BOT NICK NAME", global.GoatBot.config.nickNameBot || "Aizen V2 - Advanced Messenger Bot");
                        ...
                        async function callBackListen(error, event) {
                                ...
                                if (event.messageID && event.type == "message") {
                                        if (storage5Message.includes(event.messageID))
                                                return; // duplicate message — skip silently
                                        else
                                ...
                                if (hasBanned === false) {
                                        if (!handlerAction) {
                                                log.err("HANDLER", "handlerAction not loaded, skipping event");
                                                return;
                                        }
                                        try {
                                                await handlerAction(event);
                                        } catch (e) {
                                                log.err("HANDLER", "handlerAction error: " + e.message, e);
                                        }
                                } else
