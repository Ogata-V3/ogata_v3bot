...
// ─────────────────────────────────────────────────────────────
//  § 8b. GOATBOT EXTRA UTILS
// ─────────────────────────────────────────────────────────────

/**
 * loading — GoatBot V2 compatible ora-style spinner
 * Returns object with start/stop/succeed/fail methods
 */
...
/**
 * removeHomeDir — replaces home directory path with "~"
 * Used by cmd.js for display purposes
 */
...
/**
 * loadScripts — GoatBot V2 compatible script loader
 */
function loadScripts(type = "cmds", fileName, configCommands, getLang) {
  try {
    const folder = type === "cmds" ? "scripts/cmds" : "scripts/events";
    const filePath = require("path").normalize(`${process.cwd()}/${folder}/${fileName}.js`);
...
/**
 * unloadScripts — GoatBot V2 compatible script unloader
 */
function unloadScripts(type = "cmds", fileName, configCommands, getLang) {
  try {
    const folder = type === "cmds" ? "scripts/cmds" : "scripts/events";
    const filePath = require("path").normalize(`${process.cwd()}/${folder}/${fileName}.js`);
