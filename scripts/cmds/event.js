...
module.exports = {
	...
	langs: {
		vi: {
			...
			alreadExist: "⚠️ | File lệnh đã tồn tại, bạn có chắc chắn muốn ghi đè?\nThả cảm xúc bất kì vào tin nhắn này để tiếp tục",
			installed: "✅ | Đã cài đặt event command \"%1\" thành công, file lưu tại %2",
		...
		en: {
			...
			alreadExist: "⚠️ | The command file already exists, are you sure you want to overwrite?\nReact to this message to continue",
			installed: "✅ | Installed event command \"%1\" successfully, saved at %2",
	...
	onStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, commandName, event, getLang }) => {
		...
		if (args[0] == "load" && args.length == 2) {
			...
			message.reply(
				...
				"• .event load <filename>\n" +
				"• .event loadAll\n" +
				"• .event unload <filename>\n" +
				"• .event install <url> <filename>"
			);
		}
	},
