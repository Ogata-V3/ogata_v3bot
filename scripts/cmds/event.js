// --- Polyfill: prevents "File is not defined" crash on Node < 20 ---
// axios / form-data internally reference the global "File" class which
// only exists natively from Node.js v20+. On older runtimes (common on
// some hosts like Railway if NODE_VERSION isn't pinned), requiring axios
// can throw "File is not defined" and crash the whole command loader.
if (typeof globalThis.File === "undefined") {
	try {
		const { File } = require("buffer");
		if (File) globalThis.File = File;
	} catch (e) {
		// Fallback minimal shim (Blob has been global since Node v15)
...
module.exports = {
	...
	langs: {
		vi: {
			...
			fetchError: "❌ | Không thể tải nội dung từ URL, lỗi: %1"
		},
		en: {
			...
			fetchError: "❌ | Failed to fetch content from URL, error: %1"
		}
	},
		...
		try {
			if (args[0] == "load" && args.length == 2) {
				...
				if (args[0].toLowerCase() == "loadall") {
					try {
						allFile = fs.readdirSync(path.join(__dirname, "..", "events"))
							.filter(file =>
								file.endsWith(".js") &&
								!file.match(/(eg)\.js$/g) &&
						...
						return message.reply(getLang("fetchError", err.message));
					}
				} else {
					allFile = args.slice(1);
				}
				...
				for (const fileName of allFile) {
					try {
						...
						arrayFail.push(`${fileName} => ${err.name}: ${err.message}`);
					}
				}
				let msg = "";
				...
				const targetPath = path.join(__dirname, "..", "events", fileName);
				if (fs.existsSync(targetPath))
					...
					infoLoad.status == "success" ?
						message.reply(getLang("installed", infoLoad.name, targetPath.replace(process.cwd(), ""))) :
	...
	onReaction: async function ({ Reaction, message, event, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang }) {
		try {
			...
			if (infoLoad.status == "success") {
				message.reply(
					getLang("installed", infoLoad.name, path.join(__dirname, '..', 'events', fileName).replace(process.cwd(), "")),
